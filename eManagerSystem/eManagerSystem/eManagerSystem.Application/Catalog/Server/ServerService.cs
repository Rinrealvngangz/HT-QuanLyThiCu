   ...
   public class ServerService  : IServerService 
    {
       ...
       //   private readonly string strCon = @"SERVER=DESKTOP-4ICDD5V\SQLEXPRESS;Database =ExamManagement;User Id=test;password=nguyenmautuan123";
         private readonly string strCon = @"SERVER=DESKTOP-PK41LNN\SQLEXPRESS;Database =ExamManagement;Integrated security =true";
        ...
        public void Connect()
        {
            ...
            IP = new IPEndPoint(IPAddress.Any, 3355);
        ...
        public void ActiveUser(string MSSV,string ip,string name)
        {
        ...
        public void CheckActiveIpUser(string IP,string mssv)
        {
            if (clientIP != null)
            {
              string _mssv =  mssv.Split('-')[0];
                string name = mssv.Split('-')[1];
                if (clientIP.Any(ip => ip == IP) == true)
                {
                    ActiveUser(_mssv, IP,name);
        ...
        public void SendMessAll(string mess)
        {
            foreach (Socket client in clientList)
            {
                SendData sendData = new SendData
                {
                    option = Serialize("Send MessAll"),
                    data = Serialize("mess")
                };
                client.Send(Serialize(sendData));
            }
